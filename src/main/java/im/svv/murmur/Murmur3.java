...
public class Murmur3 {
 ...
 protected static long getblock(ByteBuffer key, int offset, int index) {
  ...
  /**
   * Set result as a hashed value
   */
  public static void murmur3(ByteBuffer key, int offset, int length, long seed, long[] result) {
  ...
  protected static byte[] getBytesFromLongs(long[] numbers) {
    ByteBuffer bytes = ByteBuffer.allocate(8*2);
    bytes.putLong(numbers[0]);
    bytes.putLong(numbers[1]);
    return bytes.array();
  }
   ...
   */
  public static long[] murmur3(ByteBuffer key, int length, long seed) {
    long[] result = new long[2];
    murmur3(key, 0, length, seed, result);
    return result;
  }

  public static long[] murmur3(Integer key, long seed) {
    ByteBuffer buffer = ByteBuffer.allocate(Integer.BYTES);
    buffer.putInt(key);
    return murmur3(buffer, Integer.BYTES, seed);
  }

  public static long[] murmur3(Long key, long seed) {
    ByteBuffer buffer = ByteBuffer.allocate(Long.BYTES);
    buffer.putLong(key);
    return murmur3(buffer, Long.BYTES, seed);
  }

  public static long[] murmur3(String key, long seed) {
    byte[] bytes = key.getBytes();
    return murmur3(ByteBuffer.wrap(bytes), bytes.length, seed);
  }

  public static byte[] murmur3AsBytes(long key, long seed) {
    return getBytesFromLongs(murmur3(key, seed));
  }

  public static long murmur2(ByteBuffer key, int offset, int length, long seed) {
  ...
  public static long murmur2(long key, long seed) {
    ByteBuffer buffer = ByteBuffer.allocate(8).putLong(key);
    return murmur2(buffer, 0, 8, seed);
  }

  public static long murmur2(String key, long seed) {
    byte[] bytes = key.getBytes();
    return murmur2(ByteBuffer.wrap(bytes), 0, bytes.length, seed);
  }
